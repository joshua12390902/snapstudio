...
自動去背 → VLM 識別 → LLM 場景企劃 → Diffusion 生成 → 物理合理的重新打光 → 多平台文案，
全程在單張 RTX 3090（24 GB）**本機推論**。設計核心：**LLM 與 Diffusion 在參數層級互相咬合**，
而非兩條各跑各的流水線。

`Python 3.10` · `diffusers 0.39` · `Gradio 6` · `Ollama (qwen3:32b + qwen2.5vl:32b)` · `RealVisXL` · `IC-Light` · `SAM2`

> 深度生成模型課程期末專題 ｜ 完整架構：[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) ｜ 開發歷程：[DEVLOG.md](DEVLOG.md) ｜ Agent 協作紀錄：[docs/WORKFLOW_LOG.md](docs/WORKFLOW_LOG.md)
...
**輸入** — 用手機隨手拍的原圖：香水就擺在雜亂桌面上（螢幕、線材、耳機盒），不同角度、未經整理。

| 隨手拍 ① | 隨手拍 ② |
|:---:|:---:|
| <img src="examples/showcase/raw_1.jpg" width="220"> | <img src="examples/showcase/raw_2.jpg" width="220"> |

**輸出** — 下不同口語需求，自動去背 → 產出風格迥異、產品坐進場景（非貼紙浮貼）的電商大圖：

| 「陽光 花園」 | 「窗邊灑進陽光」 | 「Loft 工業風」 |
|:---:|:---:|:---:|
| <img src="examples/showcase/hero_sunlit_garden.png" width="220"> | <img src="examples/showcase/window_sunlight.png" width="220"> | <img src="examples/showcase/loft_brick.png" width="220"> |
...
<p align="center"><img src="examples/showcase/ui.png" width="560"></p>
...
**LLM 場景企劃**
- **場景企劃** — 口語需求（「質感文青風」「陽光花園」）→ N 組方案 JSON（場景 prompt、光向、擺位、構圖）。
...
**Diffusion 生成 + 打光**
- **Inpaint-grounded 場景生成** — 鎖住商品像素，用專用 9 通道 inpaint 權重（優先 RealVisXL V4，不存在時退官方 SDXL 1.0 inpaint）在商品**周圍**生成檯面與場景，接地陰影/反光在同一次去噪自然生成。
- **主角擺放控制** — 商品大小／水平／垂直／旋轉皆可調，場景隨之重生。
- **IC-Light 光線融合 · A 護字（UI 預設開）** — 讓商品表面吃場景光、更融入，而**文字/logo 與輪廓邊保持原始銳利**（純 CV 偵測標籤區護住，不可靠時自動退回、永不糊字）。*UI 固定開啟；CLI／直接呼叫 `pipeline.process()` 預設關，需顯式傳 `harmonize=True`。*

**輸出與互動**
- **電商文案** — 蝦皮標題（含關鍵字）、五點賣點、IG 貼文 + hashtags。
- **多輪修改** — 「光再暖一點、背景換大理石」→ LLM 解析為參數差分；移動主角免 LLM（約 3.5 s）。
- **Gradio premium UI** — 暗色攝影棚風格自訂主題（單一暖琥珀強調色、Space Grotesk + Inter 字體、卡片化版面）。

---

## 系統架構

編排器 `snapstudio/pipeline.py::SnapStudio.process()` 串起「**LLM 決策 → Diffusion 執行**」一條鏈，全程做單卡 VRAM 調度：
...
**LLM ↔ Diffusion 咬合點**：`scene_prompt` 直接餵 SDXL inpaint、`light_direction` 餵程式接地陰影與 IC-Light、`worn_framing` 餵重塑取景。所有 LLM 輸出強制 JSON → pydantic 驗證 → 重試 → 失敗套預設模板，**系統永不因 LLM 掛掉而卡死**。
...
ollama pull qwen2.5vl:32b    # 視覺：商品識別（首次執行自動建立限制 context 的 -ctx8k 變體）
...
> 瓶頸是本機 LLM 推論：文字主力 qwen3:32b 比 14b 更聽話但約慢一倍（場景企劃數十秒）；VLM 識別與品質把關各載入一次 VLM、冷啟動較久。填「手動商品描述」可跳過識別那次 VLM 載入、縮短首次生成時間。

**IC-Light A 護字（before / after）** — 開啟後產品表面吃到場景光、更融入，文字/logo 仍維持原始銳利：

<p align="center"><img src="examples/showcase/iclight_ab.png" width="600"></p>
...
│   ├── reshape.py           # 重塑模式（RealVisXL V5 text2img + IP-Adapter + 真實細節面合成）
