...
class LLMClient:
    ...
    def _ensure_pickle(self) -> bool:
        ...
        if not config.LLM_USE_REMOTE:  # 預設關：不依賴飄忽的遠端，直接走本機 qwen3:32b
            return False
    ...
    def plan_scenes(self, card: ProductCard, user_brief: str,
        ...
        if mode == "reshape":
            ...
            scene_rule = (
                "你是商品攝影美術指導，依專業判斷決定構圖（以下是原則，不必逐字照抄）：\n"
                "1.【產品是英雄】產品要是全圖最大、最清晰、最吸睛的主體，置中偏下"
                "(product_x≈0.5, product_y≈0.65)；product_scale 給 hero 尺寸(瓶/罐/鞋≈0.48~0.56、"
                "皮夾/唇膏/耳機/戒指等小物≈0.34~0.44)。背景與道具只是襯托，不可比產品更大或更搶眼。\n"
                "2.【忠實反映使用者需求】(最高優先) 使用者描述的場景元素與光線都要清楚出現在每個方案裡；"
                "沒提到的才自由發揮。若使用者點名某種光線(陽光灑落/窗光/逆光/夕陽)，就把它畫成**明顯可見"
                "的主角級光效**(visible sunbeams / god rays / strong directional window light)，別淡化成平光。\n"
                "3.【接地與光影】產品站在它正下方合理的抬高檯面(marble/wood/concrete/metal tabletop, "
                "pedestal, ledge)，不可浮空、不可擺在地面或草地；寫 'soft contact shadow, ambient "
                "occlusion at the base'。**單一光源**：陰影只在主光同側，不要雙影或反向影。\n"
                "4.【產品要跳出來】產品輪廓與背景要有明確對比、正前方不可有遮擋物；背景偏淺或逆光時加 "
