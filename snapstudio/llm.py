    ...
    def plan_scenes(self, card: ProductCard, user_brief: str,
        ...
        if mode == "reshape":
            ...
            scene_rule = (
                ...
                "別只用形容詞。\n"
                "- ★★【使用者點名的光要看得見】當需求描述明確提到某種**光線現象**（如『陽光從窗戶灑落』"
                "/『窗光』/『逆光』/『夕陽』/『戲劇光』），scene_prompt 與 light_desc 務必把它寫成**強烈、"
                "明確可見的主角級光效**，不要降級成 'soft diffused / gentle' 平光——例『陽光從窗戶灑落』→ "
                "'bright warm sunbeams streaming in through a window, visible light rays / god rays, a glowing "
                "sunlit patch falling across the tabletop, strong directional golden window light, gentle lens "
                "glow'。光要有方向性、有明暗對比、在檯面/牆面打出明顯亮斑與柔和影子（仍維持**單一光源、"
                "陰影同一側**，不可雙影）。使用者沒點名光線時才用一般柔光。\n"
                "- **只有使用者明確要『純白／白底』時**才寫 'pure white seamless "
